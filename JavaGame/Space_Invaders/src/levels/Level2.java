...
import levels.Level1.Point;

//import main.OwnImage;
//import main.OwnImage;
//import main.MyMonsters;
...
/**
 * This class is used for the second level.
 */
public class Level2 {
	...
	public Level2() {
		...
		try {
			...
			bossAlien = new Image(new FileInputStream("BossAlien.png"));
	...
	public void setMonsterList() {
		...
		boss = new Enemy(bossAlien, 330, -120, 80.0, 80.0, 10);
	}
	...
	public void catchPowerUps() {
		...
		if (green != null) {
			if (green.getX() >= spaceshipPosition.x && green.getX() <= spaceshipPosition.x + 90
				...
				images.add(new OwnImage(green.getImage(), green.getX(), green.getY(), green.getWidth(), green.getHeight()));				green.setY(6);
			}
	...
	public void monsterHitIdentifier(GameModel model) {
		...
		for (Enemy enemy : enemys) {
			...
			if (enemy.getY() >= 650) {
				...
				hst.addtofile(model.getScore(), model.getName(), model.getFile2());
				break;
			}
		}
		...
		for (MyShot myShot : myShots) {
			...
			if (myShot.getX() >= boss.getX() && myShot.getX() <= boss.getX() + 80 && myShot.getY() <= boss.getY() + 80
					&& myShot.getY() >= boss.getY() && boss.getHealth() >= 0) {
				myShots.remove(myShot);
				images.add(new OwnImage(hitAnimation, boss.getX() - 20, boss.getY() - 20, 120, 120));
				if (boss.getHealth() > 1) {
					boss.setHealth();
					model.setScore(10);
		...
		if (model.getScore() == 1270) { // max score
	...
	public void playerHitAction(GameModel model) {
		if (playerHit) {
			...
			if (model.getLives() > 1) {
				...
				myModel = model;
				hst = new HighScoreTable(model);
				hst.readfromfile(myModel.getFile2());
				hst.addtofile(model.getScore(), model.getName(), model.getFile2());
			}
